# ExchangeOnlineMailTrafficReports.ps1
...
function Get-MailTrafficReport-Percent { 
    ...
    if ($Category -ne "MailTrafficReport") {
        ...
        # Groups name field and adds them together and creates a total count
    ...
    # Counts every value and adds them for % calculation then does it in the next command.
...
function Get-QuarantineMessage-Sender-Percent { 
    Param (
        [Parameter(Mandatory=$false)]
        # Date 'MM/dd/yyyy'
        [String]$StartReceivedDate = (Get-Date).addMonths(-1)
	...
	# Group together and counting of top SenderAddress
